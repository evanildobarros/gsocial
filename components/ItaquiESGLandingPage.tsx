...
export const ItaquiESGLandingPage: React.FC<ItaquiESGLandingPageProps> = ({ onLoginClick, onNavigate }) => {
    ...
    return (
        <div ref={containerRef} className="h-screen overflow-y-auto bg-white dark:bg-black font-sans text-black dark:text-white selection:bg-green-100 scroll-smooth">
            ...
            {/* Accessibility Bar */}
            <div className={`fixed top-0 w-full z-[60] py-1 bg-black text-white text-[9px] font-black uppercase tracking-widest px-8 flex justify-end gap-6 transition-all ${scrolled ? 'translate-y-[-100%]' : 'translate-y-0'}`}>
                <button onClick={() => setFontSize(prev => Math.min(prev + 10, 150))} className="hover:text-green-400">Zoom +</button>
                <button onClick={() => setFontSize(prev => Math.max(prev - 10, 80))} className="hover:text-green-400">Zoom -</button>
                <button onClick={() => setContrast(!contrast)} className="flex items-center gap-1 hover:text-green-400">
            ...
            {/* Header / Navbar */}
            <header className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 dark:bg-black/90 backdrop-blur-xl border-b border-gray-100 dark:border-white/5 py-4' : 'bg-transparent py-8 top-6'}`}>
                <div className="container mx-auto px-8 flex items-center justify-between">
                    <div className="flex items-center gap-4 cursor-pointer">
                        <img 
                            src="/logo_itaqui.png" 
                            alt="Porto do Itaqui" 
                            className={`h-12 w-auto object-contain transition-all ${!scrolled && !contrast ? 'brightness-0 invert' : ''}`} 
                        />
                    </div>
                    ...
                    <nav className="hidden lg:flex items-center gap-10">
                        {['Compromissos', 'Transparência', 'Notícias'].map((item) => (
                            <button
                                ...
                                className={`text-[11px] font-black uppercase tracking-widest transition-colors hover:text-green-600 ${scrolled ? 'text-black' : 'text-white/60'}`}
                            >
                                {item}
                            </button>
                        ))}
                    ...
                    <button className="lg:hidden text-black dark:text-white" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
