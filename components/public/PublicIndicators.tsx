                    ...
                    <div className="flex flex-col lg:flex-row gap-20 items-center text-left text-black">
                        <div className="lg:w-2/5 space-y-8">
                            ...
                            <div className="p-8 bg-purple-50 rounded-[40px] border border-purple-100 flex items-start gap-4">
                                ...
                                <p className="text-sm font-bold text-purple-900 italic leading-relaxed">
                                    "O cruzamento desses dados permite priorizar investimentos em áreas de alto impacto socioambiental e baixa resiliência financeira, garantindo integridade e perenidade ao porto."
                                </p>
                            </div>
                        </div>

                        <div className="lg:w-3/5 w-full h-[600px] bg-gray-50 rounded-[80px] border border-gray-100 relative p-12 overflow-hidden shadow-2xl group transition-all hover:border-purple-200">
                            ...
                            <div className="h-full w-full relative">
                                ...
                                <div className="text-[10px] font-black absolute bottom-4 left-1/2 -translate-x-1/2 uppercase tracking-widest text-black/40">Impacto Financeiro (Econômico)</div>
                                <div className="text-[10px] font-black absolute left-2 top-1/2 -translate-y-1/2 -rotate-90 uppercase tracking-widest text-black/40">Impacto Socioambiental (ESG)</div>

                                {/* Animated Data Nodes */}
                                <Node x="80%" y="15%" color="bg-green-500" label="Emissões GEE" delay="0s" />
                                <Node x="90%" y="40%" color="bg-orange-500" label="Rel. Comunitário" delay="0.5s" />
                                <Node x="65%" y="65%" color="bg-blue-500" label="Recursos Hídricos" delay="1s" />
                                <Node x="55%" y="25%" color="bg-purple-500" label="Ética e Compliance" delay="1.5s" />
                                <Node x="30%" y="80%" color="bg-indigo-500" label="Direitos Humanos" delay="2s" />
                            </div>
                        </div>
                    </div>
                </div>
...
const Node = ({ x, y, color, label, delay }: any) => (
    <div className={`absolute flex flex-col items-center gap-2 group cursor-pointer animate-in zoom-in-50 duration-700`} style={{ left: x, top: y, animationDelay: delay }}>
        ...
        <span className="px-3 py-1 bg-white shadow-xl rounded-full text-[9px] font-black uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap text-black">{label}</span>
    </div>
);
...
const KpiDetail = ({ icon, value, label, trend, color }: any) => (
    <div className="bg-white p-12 rounded-[40px] border border-gray-100 shadow-xl hover:-translate-y-2 transition-all text-center">
        ...
        <div className="text-5xl font-black tracking-tighter mb-2 text-black">{value}</div>
