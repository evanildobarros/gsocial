        ...
        >
            <DialogTitle className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-zinc-900/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <Typography className="font-bold text-gray-900 dark:text-white leading-tight">Importar Mapeamento ESG</Typography>
                        <Typography className="text-[10px] text-gray-500 uppercase tracking-widest mt-0.5">Categorização obrigatória para inteligência de dados</Typography>
                    </div>
                </div>
                <IconButton onClick={handleClose} size="small" className="text-gray-400 hover:text-gray-600 dark:hover:text-white">
                    <X size={20} />
                </IconButton>
            </DialogTitle>
            ...
            <DialogContent className="p-6 space-y-8">
                ...
                <div className="space-y-4">
                    <Typography className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                    ...
                    {!selectedFile ? (
                        ...
                        >
                            ...
                            <Typography className="text-sm font-bold text-gray-700 dark:text-gray-200 mb-1">Arraste e solte ou clique para selecionar</Typography>
                        ...
                        <div className="bg-green-500/5 dark:bg-green-500/10 rounded-lg border border-green-500/20 p-5 flex items-center justify-between animate-in zoom-in-95 duration-200">
                            <div className="flex items-center gap-4 truncate">
                                ...
                                <div className="truncate">
                                    <Typography className="text-sm font-black text-gray-800 dark:text-gray-200 truncate">{selectedFile.name}</Typography>
                                    <Typography className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{(selectedFile.size / 1024).toFixed(1)} KB</Typography>
                                </div>
                            </div>
                            <IconButton onClick={() => setSelectedFile(null)} size="small" className="text-gray-400 hover:text-red-500">
                    ...
                    <div className={`p-5 rounded-xl border transition-all animate-in slide-in-from-top-2 ${uploadState.status === 'processing' ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-100' :
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1">
                                <Typography className="text-sm font-black text-gray-900 dark:text-gray-100 uppercase tracking-tight">
                                    {uploadState.status === 'success' ? 'Importação Concluída' : uploadState.status === 'error' ? 'Falha na Importação' : 'Processando...'}
                                </Typography>
                                <Typography className="text-[11px] text-gray-600 dark:text-gray-400 font-medium mt-1">{uploadState.message}</Typography>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <DialogActions className="p-6 border-t border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-zinc-900/50 flex justify-between gap-4">
                <Button
                    onClick={handleClose}
                    className="text-sm font-black text-gray-500 hover:text-gray-800 dark:hover:text-white uppercase tracking-widest"
                >
                    Cancelar
                </Button>
                <Button
                    ...
                    `}
                >
                    {uploadState.status === 'processing' ? <CircularProgress size={16} color="inherit" /> : <Database size={16} />}
